...
<head>
    ...
    <!-- DNS preconnect：提前建立 CDN 連線，加速後續資源載入 -->
    ...
    <link href="index.css?v=<?= filemtime('index.css') ?>" rel="stylesheet">
    <link href="common/lottery/lottery.css?v=<?= filemtime('common/lottery/lottery.css') ?>" rel="stylesheet">
    <!-- Loading overlay：JS 未就緒時遮罩，防止使用者點擊無反應（純 CSS，不依賴 Bootstrap） -->
    ...
    <!-- JS：defer 放 head，瀏覽器立刻開始下載，與 HTML 解析同步進行 -->
    ...
    <script src="common/lottery/lottery.js?v=<?= filemtime('common/lottery/lottery.js') ?>" defer></script>
</head>
<body>
<!-- Loading overlay -->
...
<script>
// Overlay 移除：獨立 script，不受其他初始化錯誤影響
(function(){
